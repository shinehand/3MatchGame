...
- 같은 contract smoke는 `Pending`/`Blocked` 잔존, evidence 누락/빈 파일, Rescue Book cosmetic equip 필수 행 삭제, 현재 HEAD와 다른 report build commit, 현재 HEAD와 다른 debug/release/device/manual evidence commit, placeholder evidence commit, Android device evidence `BLOCKED`, release install `NOT_REQUESTED` fixture가 반드시 실패하는지도 검증한다.
...
- Home snapshot은 `LevelPathPreview`, `PathNode1`, `HomePathConnector`가 렌더되고 current `GO` 라벨이 줄바꿈 없이 상용 path candy 크기를 유지하는지 검증한다.
...
- Home `LevelPathPreview`는 고립된 작은 marker가 아니라 connector와 candy dot이 있는 진행 경로로 보여야 하며, `PathNode1`의 `GO` 텍스트가 세로로 깨지면 실패해야 한다.
...
- 결과/실패 overlay는 보상/다음 행동 또는 실패 원인/다음 한 수 중심으로 6개 이하의 visible line을 유지하고, 보상/목표/핵심/다음 행동은 `OverlayChipGrid` 칩으로 분리되어 debug log처럼 과밀해지지 않아야 한다. Landscape 결과/실패 overlay는 작은 확인창이 아니라 중앙 reward/failure card로 읽히는 panel/CTA 크기를 유지해야 한다.
...
- Portrait Collection은 3열 표가 아니라 2열 앨범 카드로 보이고, 상단 reward action은 3열 compact ribbon으로 유지되며, 카드의 동물 preview가 이름/상태/장착 칩보다 먼저 읽혀야 한다.
- 잠금 카드도 무거운 회색 덩어리가 아니라 낮은 채도의 파스텔 카드로 보이되, `Stage N 해금` 상태와 동물 실루엣은 판독 가능해야 한다.
...
- Stage 1 성공 overlay는 골드/도감 토큰/Zoo-Zoo Time 보상이 있는 경우 이를 reward chip으로 표시하고, Zoo-Zoo Time이 없는 runtime 성공 케이스는 골드/도감 토큰 chip을 유지한다.
- 실패 팝업에서 재도전, 지도, 닫기 같은 비구매 선택지가 항상 보인다.
- Near Miss 실패 팝업의 `+3 이동 받고 계속` CTA는 실제로 overlay를 닫고 이동 3회를 지급해 플레이 상태로 복귀한다.
- 실패 overlay는 수집 동물 미달, 점수 미달, 덤불 미달의 `놓친 핵심` 목표와 `다음 한 수` 재도전 힌트를 action chip과 본문 양쪽에서 분리해 보여 준다.
