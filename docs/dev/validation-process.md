...
- 4매치 특수 블록이 생성된다.
- 5매치 특수 블록이 생성된다.
- 자동 headless fixture가 row+column, row+row, column+column, row+bomb, column+bomb, bomb+bomb 6종 특수 조합을 중복 제거 없이 검증하고, Stage 31 row+column 실제 swap smoke가 이동 수/점수/장애물/`is_busy` 복귀를 검증한다. `FxLayer` smoke는 특수 조합 전용 flash/ring/label VFX 생성도 확인한다.
