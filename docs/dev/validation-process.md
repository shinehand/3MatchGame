...
- Render snapshot smoke: `390x844`, `844x390`에서 Home, Home Settings Overlay ON/OFF, Home Live Event ended detail, Stage Select World Map, Stage Select World Progress, Stage Popup, Stage 4 Gameplay Buddy HUD 4상태, Stage 1 성공 결과 overlay, Stage 25 실패 overlay, Stage 31 특수 조합 6종, Collection을 실제 PNG 40장으로 저장하고 non-blank/varied pixel, 핵심 UI region, Home/World Map PLAY CTA 이미지 크기, Settings panel/toggle/close 상태, Stage Select current/cleared/locked/finale 노드 상태와 selected panel 목표/이동/보상 chip, Gameplay `StageIntroCard`/READY chip, Home 이벤트 상세 상태/CTA와 panel/progress/reward/claim PNG 최소 크기, Buddy 라벨/게이지/analytics, landscape HUD 실제 PNG 최소 크기와 board-HUD gap, landscape support card/CTA 렌더, 성공/실패 결과 overlay panel/mascot/chip/CTA PNG 최소 크기, 특수 조합 label/flash/ring 렌더, Collection header/ribbon compactness, `rabbit` 카드/preview와 locked dog card/preview/status PNG 크기를 확인
