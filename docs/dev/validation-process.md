...
- Stage Select selected panel readability gate: 월드맵 하단 `WorldSelectedPanel`이 빈 흰 tray처럼 보이지 않도록 `WorldSelectedChipRow`의 목표/이동/보상 chip, info column, `PLAY` CTA 비겹침과 PNG 최소 크기를 확인
- Meta modal/card readability gate: Collection portrait가 2열 앨범 카드로 읽히고 header/cosmetic ribbon이 표처럼 길어지지 않는지, landscape 첫 카드가 얇은 row로 무너지지 않는지, 동물 preview가 충분히 큰지, Stage Popup landscape panel과 `PLAY` CTA가 상용 start modal 위계를 유지하는지 확인
- Home event detail readability gate: Live Event 상세가 작은 정보 상자로 축소되지 않도록 `EventDetailPanel`, progress card, reward chip, claim CTA의 portrait/landscape PNG 최소 크기와 긴 문구 stress bounds를 확인
- Home level path readability gate: `LevelPathPreview`가 고립된 작은 marker가 아니라 connector와 candy dot이 있는 진행 경로로 보이고, current `GO` 라벨이 줄바꿈 없이 판독되는지 확인
- Stage Select world path readability gate: landscape 월드맵이 작은 점선 경로처럼 보이지 않도록 `WorldStageNode*`, current node, path connector, candy dot의 실제 PNG 크기와 viewport bounds를 확인
- Gameplay start intro readability gate: `StageIntroCard`가 보드 중심의 compact start badge로 보이고 `LEVEL/READY` multi-line raw label 또는 landscape HUD 침범으로 돌아가지 않는지 확인
- Result overlay density gate: Stage 1 성공 overlay와 Stage 25 near-miss 실패 overlay의 본문 visible line을 6개 이하로 유지하고, 보상/토큰/Zoo-Zoo Time 또는 실패 목표/핵심/다음 한 수를 `OverlayChipGrid` 칩으로 분리하며, landscape 결과 카드가 작은 확인창으로 축소되지 않는 PNG 최소 크기를 확인
...
- Render snapshot smoke: `390x844`, `844x390`에서 Home, Home Settings Overlay ON/OFF, Home Live Event ended detail, Stage Select World Map, Stage Select World Progress, Stage Popup, Stage 4 Gameplay Buddy HUD 4상태, Stage 1 성공 결과 overlay, Stage 25 실패 overlay, Stage 31 특수 조합 6종, Collection을 실제 PNG 40장으로 저장하고 non-blank/varied pixel, 핵심 UI region, Home/World Map PLAY CTA 이미지 크기, Home `LevelPathPreview`/`PathNode1`/connector PNG 크기, Settings panel/toggle/close 상태, Stage Select current/cleared/locked/finale 노드 상태와 world node/current node PNG 최소 크기, selected panel 목표/이동/보상 chip, Gameplay `StageIntroCard`/READY chip, Home 이벤트 상세 상태/CTA와 panel/progress/reward/claim PNG 최소 크기, Buddy 라벨/게이지/analytics, landscape HUD 실제 PNG 최소 크기와 board-HUD gap, landscape support card/CTA 렌더, 성공/실패 결과 overlay panel/mascot/chip/CTA PNG 최소 크기, 특수 조합 label/flash/ring 렌더, Collection header/ribbon compactness, `rabbit` 카드/preview와 locked dog card/preview/status PNG 크기를 확인
...
`.github/workflows/no-device-alpha-gate.yml`은 `main` push, pull request, 수동 실행에서 Godot 4.6.1과 Xvfb 렌더 preflight 의존성을 설치하고 아래 no-device 검증을 실행한다.
...
`zsh scripts/validate_provider_readiness.sh`는 `data/provider_readiness.json`이 analytics `local_buffer`와 monetization `local_simulator`의 provider-neutral 계약을 실제 코드 상수와 맞게 고정하는지 검사한다. 이 gate는 `OPEN-007` 실제 SDK 공급자 선택을 완료했다는 뜻이 아니라, Firebase/GameAnalytics/AdMob/Google Play Billing/Unity Ads/AppLovin/ironSource/RevenueCat 같은 실제 provider명이 공백/구분자 표기로 manifest에 섞이는지 막고, SDK 연결 전 adapter hook, source/result canonicalization, queue/rejected_contract, request log, provider_result 보존 규칙을 CI가 지키게 하는 준비도 검사다.
...
`zsh scripts/validate_render_snapshots.sh`는 표시 렌더러가 있는 로컬 환경에서는 일반 Godot 실행, CI에서는 `xvfb-run`으로 Home, Home Settings Overlay ON/OFF, Home Live Event ended detail, Stage Select World Map, Stage Select World Progress, Stage Popup, Stage 4 Gameplay Buddy HUD 4상태, Stage 1 성공 결과 overlay, Stage 25 실패 overlay, Stage 31 특수 조합 6종, Collection 스냅샷을 `390x844`와 `844x390` PNG 정확히 40장으로 저장한다. Home 스냅샷은 `HomePlayButton`, `LevelPathPreview`, `PathNode1`, `HomePathConnector`, `BottomNav` 픽셀과 path node PNG 최소 크기를 확인한다. Home Settings Overlay 스냅샷은 ON/OFF 상태별 설정 panel, title, auto-save summary, sound/haptics toggle, close CTA의 픽셀, bounds, 텍스트, 최소 이미지 크기를 확인한다. Stage Select World Map/Progress 스냅샷은 Stage 1-3 클리어, Stage 4 current, Stage 5 locked fixture에서 current ring/PLAY ribbon, cleared star tray, locked badge, finale ribbon, world node/current node PNG 최소 크기, 하단 선택 패널 비겹침, `WorldSelectedChipRow`의 목표/이동/보상 chip 텍스트와 PNG 최소 크기를 확인한다. Home Live Event ended detail 스냅샷은 `active`/`offline`/`upcoming`/`ended` 상태 배지와 CTA 계약을 먼저 확인한 뒤, 종료 상태의 `EventDetailOverlay`, 상태 배지 텍스트, 진행 카드 문구, 보상 칩 텍스트, `EventDetailPanel`/`EventDetailProgressCard`/reward chip/`EventClaimButton` PNG 최소 크기, `EventClaimButton` disabled/`종료됨` 상태를 확인한다. Stage 4 Buddy 스냅샷은 0/3, 2/3, 출동, 완료 상태의 portrait 라벨/게이지, `StageIntroCard`/`StageIntroStatusChip`, landscape `LandscapeHudShell`/combo text, shell이 PNG 너비의 20% 이상을 확보하고 `BoardFrame`과의 gap이 PNG 너비의 14% 이하인지, `LandscapeSupportCard`와 landscape action CTA가 실제 PNG에 렌더되는지, `buddy_skill_charge`/`buddy_skill_ready`/`buddy_skill_trigger` payload를 확인한다. Stage 1 성공 스냅샷은 실제 클리어 후 보상/별/Zoo-Zoo Time/`다음 스테이지`/`홈으로` CTA, `OverlayChipGrid` 보상 칩, 특히 landscape에서 작은 modal로 축소되지 않는 panel/mascot/chip/CTA PNG 최소 크기, `stage_complete` payload, Stage 2 해금을 확인한다. Stage 25 실패 스냅샷은 near-miss 본문과 목표/핵심/다음 action chip, `+3 이동 받고 계속`/`재도전` CTA, 특히 landscape에서 작은 modal로 축소되지 않는 panel/mascot/chip/CTA PNG 최소 크기를 확인한다. Stage 31 조합 스냅샷은 실제 `_resolve_swap` 발동 직후 조합별 label/flash/ring 렌더, explosive echo ring 필요 여부, filename combo type, `special_combo_trigger` payload, transient VFX cleanup을 확인한다. Collection 스냅샷은 `rabbit` 40토큰과 `rabbit_sprout_frame` 장착 fixture에서 compact `CollectionHeaderPanel`/`CosmeticEquipGrid`, 상세 reward track, `장착중` 버튼 region, `rabbit` 카드와 `AnimalPreview`, locked dog 카드/preview/status의 PNG 크기와 `Stage 7 해금` 문구를 확인한다. 이 gate는 blank/transparent/offscreen/missing texture 회귀와 수동 QA 전 Home Settings/Stage Select 진행 상태/Home 이벤트 상세/Buddy/결과/특수 VFX/Collection 장착 UI 판독성 회귀를 잡기 위한 no-device preflight이며, 실기기 screenshot/video/logcat evidence와 실제 소리/햅틱 체감을 대체하지 않는다. GitHub-hosted runner의 Xvfb renderer가 실패할 수 있어 workflow에서는 non-blocking artifact attempt로 실행하고, `validate_gameplay.sh`는 GitHub Actions 안에서 이 단계만 skip caveat를 출력한다. 로컬 또는 지원되는 Xvfb 환경에서는 `zsh scripts/validate_render_snapshots.sh`를 blocking으로 실행해야 한다.
...
`zsh scripts/validate_alpha_qa_report.sh --report=output/alpha-lock-pass/YYYY-MM-DD/alpha-lock-pass-manual-qa-YYYY-MM-DD.md`는 수동 QA 보고서의 최종 승인 직전 게이트다. 템플릿 placeholder, `Pending`/`Fail`/`Blocked`/`Open` 결과, `Overall result: Pass`와 `QA result: Approve` 불일치, 현재 HEAD와 맞지 않는 build commit, 비어 있거나 존재하지 않는 evidence path가 남아 있으면 실패한다. 또한 템플릿의 핵심 섹션, 대표 코스, stage smoke, focused gate, Rescue Book cosmetic equip, Stage 31 combo, Rescue Buddy, monetization/analytics 시나리오 행과 evidence anchor가 삭제되었거나 필수 행의 Result가 `Pass`가 아닌 보고서도 실패한다. `android-debug-export.txt`, `android-release-export.txt`, `android-device-evidence.txt`, `device-info.txt`, `install-log.txt`, `release-install-log.txt`, `release-run-log.txt`는 파일 존재뿐 아니라 내부 PASS/Success 문구, release checksum, launch/screenshot/screenrecord/logcat 개별 PASS, 필수 device field 값, `Window size`의 pixel size도 검사한다. `android-debug-export.txt`, `android-release-export.txt`, `android-device-evidence.txt`의 `Commit`과 `manual-device-checks.txt`, sound/haptics/touch note 파일의 `Build source commit`은 현재 HEAD의 7-40자 SHA prefix와 일치해야 한다. `manual-device-checks.txt`와 sound/haptics/touch note 파일은 `Manual result: PASS`, tester, device, OS, note가 있어야 통과한다. 이 검사는 작성 중인 보고서에는 의도적으로 실패하므로 일반 `validate_gameplay.sh`에는 포함하지 않는다.

`zsh scripts/validate_alpha_qa_report_contract.sh`는 실제 기기나 keystore 없이 임시 alpha report/evidence fixture를 만들어 `validate_alpha_qa_report.sh`의 계약을 검증한다. PASS fixture는 현재 HEAD, `Overall result: Pass`, `QA result: Approve`, 필수 evidence content와 evidence 내부 commit을 채워 통과해야 하며, `Pending` 잔존, evidence 누락, Rescue Book cosmetic equip 필수 행 삭제, 잘못된 report commit, stale debug/release/device/manual evidence commit, placeholder evidence commit, `Capture result: BLOCKED`, release `Install result: NOT_REQUESTED` fixture는 반드시 실패해야 한다. 이 contract smoke는 일반 `validate_gameplay.sh`와 no-device CI에 포함된다.
...
- Mobile viewport matrix는 Stage 4 세로 HUD의 `BoardFrame`, `HudTopDock`, `HudGoalDock`, `HudBoosterDock`, `HudBuddyGauge`, `PortraitGoalSummary` bounds, HUD/보드 겹침, 보드와 하단 booster dock 사이 과도한 빈 공간을 검사한다. Stage intro는 `StageIntroCard`가 compact badge 높이를 유지하고 raw multi-line `LEVEL/READY` label로 돌아가지 않는지 확인한다. Stage Popup portrait 최소 폭, 결과/실패 overlay 본문 visible line 제한, landscape `LandscapeHudShell` 최소 폭/보드 비겹침과 board-HUD gap, landscape BoardFrame 과신장도 함께 막아 상용 start card와 보드 중심 레이아웃이 유지되는지 확인한다.
...
- Stage 1 클리어 판정 경로에서 결과 overlay가 표시되고, 보상/별/점수/다음 행동, `OverlayChipGrid` 보상 칩, `다음 스테이지`/`홈으로` CTA를 보여 준다.
- Stage 1 FTUE 실패 판정 경로에서 실패 overlay는 `무료 재도전` CTA를 보여 주고 보상형 광고/IAP 문구를 노출하지 않는다.
- Stage 25 near-miss 실패 판정 경로에서 실패 overlay가 표시되고, 실패 유형, 남은 목표, `놓친 핵심`, `다음 한 수`, action chip, 추천 부스터, `+3 이동 받고 계속`/`재도전` CTA를 보여 준다.
- 실패 overlay helper smoke는 수집 동물 미달, 점수 미달, 덤불 미달의 `놓친 핵심`/`다음 한 수` 문구를 분리 검증한다.
- Mobile viewport matrix는 Stage 25 실패 overlay panel/title/body/primary/secondary CTA bounds와 body-to-CTA overlap을 portrait/landscape 분기에서 검사한다.
...
- `MonetizationGateway` adapter result alias는 `success -> completed`, `cancelled/timeout/error/unknown -> failed`, `started/in_progress -> pending`으로 정규화되어 gameplay에는 `completed`, `failed`, `pending`만 전달된다. canonical 결과와 다른 원 provider 문자열은 `details.provider_result`로 보존해 IAP cancel/restore analytics 분기를 잃지 않는다.
