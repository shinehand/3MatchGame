...
- 런타임 analytics가 `GameSession` 저장과 `AnalyticsGateway` local_buffer queued dispatch 양쪽에 남고, local_buffer가 disk reload 후에도 유지되며, 계약 위반 이벤트는 provider queue 대신 rejected_contract로 격리되는지 확인
