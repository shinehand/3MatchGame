...
- 스테이지 선택 씬에서 배경 위 마스코트가 떠 있고 현재 스테이지 카드가 펄스되어 월드맵처럼 보이는지 확인
- 스테이지 카드를 누르면 바로 게임으로 가지 않고 Stage Popup이 뜨는지 확인
- Stage Popup이 페이드/팝인으로 뜨고 닫을 때 자연스럽게 사라지는지 확인
- Stage Popup에서 목표, 이동 수, 보상, 시작 부스터 3종, START 버튼이 정상 표시되는지 확인
