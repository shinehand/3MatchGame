...
echo "[1/14] Stage data structure validation"
zsh scripts/validate_stage_data.sh

echo "[2/14] Stage balance validation"
zsh scripts/validate_stage_balance.sh

echo "[3/14] Android export config validation"
...
echo "[4/14] Provider readiness validation"
...
echo "[5/14] LiveOps config validation"
liveops_config_stdout="/tmp/puzzle-liveops-config.stdout"
rm -f "$liveops_config_stdout"
if ! zsh scripts/validate_liveops_config.sh >"$liveops_config_stdout" 2>&1; then
  echo "LiveOps config validation failed."
  cat "$liveops_config_stdout"
  exit 1
fi
cat "$liveops_config_stdout"

echo "[6/14] Godot import cache"
...
echo "[8/14] Render snapshot smoke"
...
touch "$headless_log" "$headless_stdout"
...
echo "[10/14] Texture loading anti-pattern scan"
...
echo "[11/14] Alpha QA packet integrity"
...
echo "[12/14] Alpha QA report validator contract"
...
if ! zsh scripts/validate_alpha_qa_report_contract.sh >"$alpha_report_contract_stdout" 2>&1; then
  echo "Alpha QA report validator contract failed."
  cat "$alpha_report_contract_stdout"
  exit 1
fi
cat "$alpha_report_contract_stdout"

echo "[13/14] Android QA helper contract smoke"
...
echo "[14/14] Manual smoke checklist"
cat <<'EOF'
...
- 스테이지 선택 씬 첫 화면에서 긴 설명 패널/카드 목록이 아니라 전체 월드맵, 10개 경로 노드, 큰 `PLAY` 버튼이 먼저 보이는지 확인
...
- Stage Popup에서 목표, 이동 수, 보상, 아이콘이 있는 시작 부스터 3종, PLAY 버튼이 정상 표시되는지 확인
- Stage Popup을 닫으면 overlay가 사라지고, PLAY 직전 선택 스테이지/부스터가 저장되는지 확인
- Stage 4 같은 Buddy 스테이지에서는 Stage Popup에 Rescue Buddy 동물/스킬/조건 요약이 보이고 Gameplay HUD Buddy 문구와 같은 대상을 가리키는지 확인
- 부스터를 선택하고 PLAY를 누르면 게임 시작 보드에 선택 부스터가 배치되는지 확인
...
- 게임 시작 때 보드가 살짝 등장하고 `StageIntroCard`와 READY/GO chip 시작 연출이 보드 중심에 보이는지 확인
